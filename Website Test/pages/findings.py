...
def build_figures():
    ...
    fig_egg_birds.add_trace(
        go.Scatter(x=merged["month"], y=merged["egg_price"], mode="lines", name="Egg price"),
        secondary_y=False
    )
    fig_egg_birds.add_trace(
        go.Scatter(x=merged["month"], y=merged["birds_affected"], mode="lines", name="Birds affected"),
        secondary_y=True
    )
    ...
    # overall percent change by food group (first to last month)
    ...
    # lagged correlation curve (birds lead egg % change)
    ...
    # forecast backtest (train 2020–2023, predict 2024–2026 vs actual) for food groups
    ...
    for fg in sorted(train["Food Group"].unique()):
        ...
        fig_backtest.add_trace(go.Scatter(
            x=act["month"],
            y=act["Price (Latest date)"],
            mode="lines",
            name=f"{fg} actual (2024–2026)"
        ))
        fig_backtest.add_trace(go.Scatter(
            x=pred["month"],
            y=pred["pred"],
    ...
    # train on all available data (2020–2026) and predict next 3 years (36 months)
    fg_sorted = fg_month.sort_values("month").copy()
    last_month = pd.to_datetime(fg_sorted["month"].max())
    future_months_3y = pd.date_range(last_month + pd.offsets.MonthBegin(1), periods=36, freq="MS")

    pred3_rows = []
    for fg in fg_sorted["Food Group"].unique():
        g = fg_sorted[fg_sorted["Food Group"] == fg].sort_values("month")
        if len(g) < 6:
            continue
        t = np.arange(len(g))
        y = g["Price (Latest date)"].values
        b = np.cov(t, y, bias=True)[0, 1] / np.var(t)
        a = y.mean() - b * t.mean()
        t_future = np.arange(len(g), len(g) + len(future_months_3y))
        y_future = a + b * t_future
        pred3_rows.append(pd.DataFrame({"month": future_months_3y, "Food Group": fg, "pred": y_future}))

    pred3_df = pd.concat(pred3_rows, ignore_index=True)

    fig_forecast_3y = go.Figure()
    for fg in sorted(fg_sorted["Food Group"].unique()):
        hist = fg_sorted[fg_sorted["Food Group"] == fg].sort_values("month")
        pred = pred3_df[pred3_df["Food Group"] == fg].sort_values("month")

        fig_forecast_3y.add_trace(go.Scatter(
            x=hist["month"],
            y=hist["Price (Latest date)"],
            mode="lines",
            name=f"{fg} actual (train 2020–2026)"
        ))
        fig_forecast_3y.add_trace(go.Scatter(
            x=pred["month"],
            y=pred["pred"],
            mode="lines",
            line=dict(dash="dot"),
            name=f"{fg} forecast (next 3y)"
        ))

    fig_forecast_3y.update_layout(
        title="3-Year Forecast by Food Group (Trained on 2020–2026 Data)",
        xaxis_title="Month",
        yaxis_title="Average price",
        hovermode="x unified",
        xaxis=dict(rangeslider=dict(visible=True))
...
layout = html.Div(
    ...
    children=[
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P("Key results and evidence derived from the notebooks and the event dataset.")
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P(
                "This plot compares food groups on the same timeline to show which categories rise steadily versus which categories spike. "
                "Constant increases suggest long-term inflation pressure, while short spikes suggest event-driven months that may require separate event-based analysis."
                    )
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P("The dual-axis time series preserves timing and supports inspection of alignment and possible lags.")
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P(
                "The boxplot summarizes typical prices and volatility by food group using medians, quartiles, and outliers. "
                "Wider spreads indicate categories with higher budgeting uncertainty, while frequent outliers indicate occasional shock months that can dominate short-term costs."
                    )
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P(
                 "This bar chart ranks food groups by total percent change from the first to the last available month, which answers which category increased the most overall. "
                 "A larger percent increase indicates a stronger long-term inflation burden for that category and highlights where forecasting accuracy and scenario planning matter most."
                    )
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P("This tests time dependency by evaluating whether birds affected leads egg % change by 0–6 months.")
        ]),
        ...
        html.Div(style=CARD_STYLE, children=[
            ...
            html.P(
                 "This backtest trains a baseline trend model on 2020–2023 data and compares predicted 2024–2026 values against what actually occurred. "
                 "Consistent gaps between predicted and actual values show that a simple trend is inconsistent and motivates adding event severity and other drivers to improve forecast."
                )
        ]),

        html.Div(style=CARD_STYLE, children=[
            html.H3("3-year forecast using all available data (2020–2026)"),
            dcc.Graph(figure=fig_forecast_3y),
            html.P(
                "This forecast fits a baseline trend using all historical months in the dataset and predicts 36 months into the future for each food group. "
                "The dotted forecast lines provide a scenario baseline for planning, while differences between groups show which categories are projected to grow faster under trend-only assumptions."
                    )
        ]),
